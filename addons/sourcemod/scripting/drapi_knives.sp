/*   <DR.API KNIVES> (c) by <De Battista Clint - (https://sourcemod.market)  */
/*                                                                           */
...
public Action Event_Say(int client, const char[] command, int args)
{
	...
	|| C_knives_access_mode == 2 && IsAdminEx(client))
	{
		...
		GiveKnifeSayExplode(client, 	"Knife Menu Command Push", 			"MenuKnives_PUSH_MENU_TITLE", 		516, 	text);
	}
...
void BuildMenuKnivesCT(int client)
{
	...
	Format(butterfly, sizeof(butterfly), "%T", "MenuKnives_PUSH_MENU_TITLE", client);
	AddMenuItem(menu, "516", butterfly);
		...
		switch(action)
		{
			...
			case MenuAction_Select:
			{
				...
				)
				{
					...
					else if(StrEqual(item, "516"))
					{
						Format(weaponname, sizeof(weaponname), "%T", "MenuKnives_PUSH_MENU_TITLE", param1);
					}
